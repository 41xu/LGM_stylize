...
def process(opt: Options, path):
    ...
    #     gt_image = ip2p(original_image, original_image, prompt_utils)["edit_images"].detach().clone()
    #     tmp = gt_image.squeeze(0) * 255
    #     tmp = tmp.clamp(0, 255).cpu().numpy().astype(np.uint8)
    #     Image.fromarray(tmp).save(f'{opt.workspace}/initialize_{i}.png')
    #     print(f"Step {i} Done!")
    ...
    for i in range(100):
        ...
        tmp = gt_image.squeeze(0) * 255
        tmp = tmp.clamp(0, 255).cpu().numpy().astype(np.uint8)
        Image.fromarray(tmp).save(f'{opt.workspace}/reload_{i}.png')

def process_random_seed(opt, path):
    name = os.path.splitext(os.path.basename(path))[0]
    print(f'[INFO] Processing {path} --> {name}')
    os.makedirs(opt.workspace, exist_ok=True)
    ...
    for i in range(100):
        ...
        tmp = gt_image.squeeze(0) * 255
        tmp = tmp.clamp(0, 255).cpu().numpy().astype(np.uint8)
        Image.fromarray(tmp).save(f'{opt.workspace}/seed_{i}.png')
        print(f"Step {i} Done!")
...
for path in file_paths:
    # setup_seed(0)
    # process(opt, path)
    process_random_seed(opt, path)



"""
python ip2p_test.py big --resume pretrained/model_fp16.safetensors --workspace ip2p_test --test_path style_test/
"""
