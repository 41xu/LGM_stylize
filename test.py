# test 3DGS output for depth. plan to use depth as a constrain for stylization.
import os
import tyro
import glob
import imageio
...
def process(opt: Options, path):
    ...
    with torch.no_grad():
        for view in views:
            ...
            render_res = model.gs.render(gaussians, cam_view.unsqueeze(0), cam_view_proj.unsqueeze(0), cam_pos.unsqueeze(0), scale_modifier=1)
            image = render_res['image']
            depth = render_res['depth']
            depth = depth.squeeze(0).squeeze(0).permute(1, 2, 0).contiguous().float()
            # depth_image = depth.clamp(0, 255).cpu().numpy().astype(np.uint8)
            ...
            cv2.imwrite(f'{opt.workspace}/{name}_{view}_depth.png', depth_image)
            # Image.fromarray(depth_image).save(f'{opt.workspace}/{name}_{view}_depth.png')
            image = (image.squeeze(1).permute(0, 2, 3, 1).contiguous().float().cpu().numpy() * 255).astype(np.uint8)
            Image.fromarray(image[0]).save(f'{opt.workspace}/{name}_{view}.png')
