# use face as example, debug ip2p input and output
import os
import tyro
import glob
import imageio
...
def process(opt: Options):
    ...
    #         img = cv2.imread(os.path.join(data_path, img_name))[:, :, ::-1].copy() / 255
    #         img = torch.FloatTensor(img).unsqueeze(0).to(device) # 1,H,W,C
    #         edit = ip2p(img, img, prompt_utils)["edit_images"].detach().clone() # 1,H,W,C
    #         edit = edit.squeeze(0) * 255
    #         edit = edit.clamp(0, 255).cpu().numpy().astype(np.uint8)
    #         print(img_name.split('.')[0])
    #         Image.fromarray(edit).save(f"{opt.workspace}/{img_name.split('.')[0]}.png")
    #         # Image.fromarray(edit).save(f'{opt.workspace}/{img_name.split('.')[0]}_edited.png')
    ...
    with torch.no_grad():
        for cam in camlist:
            cam.original_image = model.gs.render_face(gaussians, cam)['image'].unsqueeze(0).permute(0, 2, 3, 1)
    ...
    for step in tqdm.tqdm(range(opt.edit_train_steps)):
        ...
        if view_index not in edit_images or (opt.per_editing_steps > 0 and opt.edit_begin_step < step < opt.edit_util_step and step % opt.per_editing_steps == 0):
            render_tmp = render_image.squeeze(0) * 255
            render_tmp = render_tmp.clamp(0, 255).detach().cpu().numpy().astype(np.uint8)
            Image.fromarray(render_tmp).save(f'{opt.workspace}/{name}_rendered_{step}.png')
            print(original_image.requires_grad)
            original_tmp = original_image.squeeze(0) * 255
            original_tmp = original_tmp.clamp(0, 255).detach().cpu().numpy().astype(np.uint8)
            Image.fromarray(original_tmp).save(f'{opt.workspace}/{name}_original_{step}.png')
            result = ip2p(render_image, render_image, prompt_utils)
            edited_img = result["edit_images"].detach().clone() # 1,H,W,C
            edited_img = edited_img.squeeze(0) * 255
            edited_img = edited_img.clamp(0, 255).detach().cpu().numpy().astype(np.uint8)
            Image.fromarray(edited_img).save(f'{opt.workspace}/{name}_edited_{step}.png')
            break
    ...
    #     loss += 10 * F.mse_loss(render_image, original_image)
...
"""
python face_debug.py big --resume pretrained/model_fp16.safetensors --workspace face --data_path /home/shiyaoxu/datasets/face
"""
