...
def test_seed(seed):
    ...
    for i in range(8):
        generator = torch.Generator(device='cuda').manual_seed(seed)
        noise = torch.randn((1,2,2), generator=generator, device="cuda")
        print(noise)
        print("-----")

# process function
def process(opt: Options, path, seed):
    ...
    # 写死，先渲8张图，然后编辑，然后这张图做gt，gt固定
    edit_views = [0, 45, 90, 135, 180, 225, 270, 315]
    original_images = []
    for edit_view in edit_views:
    ...
    gt_images = []
    for i in range(len(edit_views)):
        generator = torch.Generator(device=device).manual_seed(seed)
        # gt_image = ip2p(render_images[i], original_images[i], prompt_utils, i)["edit_images"].detach().clone()
        gt_image = ip2p(render_images[i], original_images[i], prompt_utils, generator)["edit_images"].detach().clone()
        tmp = gt_image.squeeze(0) * 255
        tmp = tmp.clamp(0, 255).cpu().numpy().astype(np.uint8)
        Image.fromarray(tmp).save(f'{opt.workspace}/{name}_edited_{i}.png')
        gt_images.append(tmp)
    
    Image.fromarray(np.concatenate(gt_images, axis=1)).save(f'{opt.workspace}/{name}_edited.png')
    #     tmp = gt_image.squeeze(0) * 255
    #     tmp = tmp.clamp(0, 255).cpu().numpy().astype(np.uint8)
    ...
    # gt_image = ip2p(render_images, original_images, prompt_utils)["edit_images"].detach().clone()
    # tmp = gt_image.squeeze(0) * 255
    # tmp = tmp.clamp(0, 255).cpu().numpy().astype(np.uint8)
...
for path in file_paths:
    setup_seed(20240226)
    process(opt, path, 20240226)

"""
python stylize_fix8view.py big --resume pretrained/model_fp16.safetensors --workspace fix_front --test_path stylize_test/
"""
